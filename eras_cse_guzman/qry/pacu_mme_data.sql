...
/* 		AND ENCNTR_LOC_HIST.LOC_FACILITY_CD IN (
	...
	SELECT 
		DOSES.*,
		CASE
			WHEN DOSES.MEDICATION = 'hydromorphone' THEN DOSE * 6.7
			ELSE DOSES.DOSE
		END AS MME_IV,
	...
	WHERE
		...
		AND ORDER_PRODUCT.ITEM_ID(+) IN (
			2877313, -- oxyCODONE IR 5 mg TAB
			2962794, -- oxyCODONE 5 mg/5 ml oral LIQ
			39723170, -- HYDROmorphone 2mg/ml 1ml INJ Vial
			179696577 -- MORPhine sulfate PF 4 mg/mL INJ VL
		)
	...
	SELECT
		ENCNTR_ID,
		COUNT(DISTINCT EVENT_ID) AS NUM_IV_DOSES,
		SUM(MME_IV) AS TOTAL_MME_IV
	FROM
		DOSE_ORDERS
	WHERE
		MEDICATION IN ('morphine', 'hydromorphone')
	GROUP BY
		ENCNTR_ID
), OXYCOD_TOTALS AS (
	SELECT
		ENCNTR_ID,
		COUNT(DISTINCT EVENT_ID) AS NUM_OXYCOD_DOSES,
		SUM(DOSE) AS TOTAL_OXYCOD_MG
	FROM
		DOSE_ORDERS
	WHERE
...
SELECT
	DOSE_ORDERS.ENCNTR_ID,
	DOSE_ORDERS.NURSE_UNIT,
	CALC_MME.NUM_IV_DOSES,
	CALC_MME.TOTAL_MME_IV,
	OXYCOD_TOTALS.NUM_OXYCOD_DOSES,
	OXYCOD_TOTALS.TOTAL_OXYCOD_MG
FROM
	CALC_MME,
	DOSE_ORDERS,
	OXYCOD_TOTALS
WHERE
	DOSE_ORDERS.ENCNTR_ID = CALC_MME.ENCNTR_ID
	AND DOSE_ORDERS.ENCNTR_ID = OXYCOD_TOTALS.ENCNTR_ID(+)
