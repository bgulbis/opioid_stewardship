...
```{r, fig.cap="Patients receiving pain medications as a percent of all patients on the medicine service line.", fig.width=8}
...
```{r, fig.cap="Timeline of pain medication administration over first 30 days of admission", eval=FALSE}
df %>%
    plot_ly(
        x = ~time.admit, 
        color = ~type,
...
```{r, fig.cap="Timeline of pain medication administration over first 10 days of admission"}
df %>%
    plot_ly(
        x = ~time.admit, 
        color = ~type,
...
df %>%
    semi_join(pts_iv, by = "millennium.id") %>%
    ggplot(aes(x = time.admit, fill = type)) +
    geom_histogram(binwidth = 24) +
    # geom_dotplot(alpha = 0.5, binwidth = 24, dotsize = 0.5, method = "histodot") +
