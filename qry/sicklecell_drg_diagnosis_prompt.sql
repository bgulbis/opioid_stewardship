SELECT
    ...
    NM_ICD.SOURCE_IDENTIFIER AS DIAG_CODE,
...
FROM
    ...
    PERSON
WHERE
    NOMENCLATURE.SOURCE_IDENTIFIER IN ('811', '812', '662')
    AND NOMENCLATURE.ACTIVE_IND = 1
    AND (
        NOMENCLATURE.NOMENCLATURE_ID = DRG.NOMENCLATURE_ID
        AND DRG.ACTIVE_IND = 1
    )
    AND (
	    DRG.ENCNTR_ID = ENCOUNTER.ENCNTR_ID
    ...
    AND (
        DRG.ENCNTR_ID = DIAGNOSIS.ENCNTR_ID
        AND DIAGNOSIS.ACTIVE_IND = 1
        AND DIAGNOSIS.DIAG_TYPE_CD = 26244
    )
    AND (
        DIAGNOSIS.NOMENCLATURE_ID = NM_ICD.NOMENCLATURE_ID
        AND NM_ICD.ACTIVE_IND = 1
    )
	AND (
